...
void TFT::init()
{
  ...
  if (tft_info) tft_info("init ST7789");
	writeCommand(0x11);
	delay(5);
	writeCommand(0x36);
	SPI.write(0x00);
...
void TFT::startBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (_rotation == 1) {
    writeCommand(0x36);  // 90
    SPI.write(0x60);
  }
  if (_rotation == 2) {
    writeCommand(0x36);    // 180
    SPI.write(0xC0);
  }
  if (_rotation == 3) {
    writeCommand(0x36);    // 270
    SPI.write(0xA0);
  }
  writeCommand(0x2C);
  endWrite();
...
***************************************************************************************/
void TFT::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data)
{
...
bool TFT::setCursor(uint16_t x, uint16_t y) {
  ...
  writeCommand(0x2A);
  SPI.write(x >> 8);  //Column Start
  SPI.write(x & 0xFF);
  writeCommand(0x2B);
  SPI.write(y >> 8);  //Row Start
  SPI.write(y & 0xFF);
  _curX = x;
  _curY = y;
...
{
  uint16_t len = 0;
  while (str[len] != 0)len++; // determine length
  //log_i("%i", strlen);
  ...
  if (_f_curPos == true) {
    tmp_curX = _curX;  //new CursorValues?
    tmp_curY = _curY;
    _f_curPos = false;
  }
  ...
  while (i != len) //until string ends
  {
      ...
      while ((str[a] != 32) && (a < len)) {
        ...
        if ((_f_utf8) && (str[a] >= 0xC2)) { //next char is UTF-8
          ...
          if ((ch < 0xD4B0)) { // char is in range, is not a armenian char or higher
            xC = (str[a] - 0xC2) * 64;
            a++;
            fi += (str[a] + xC - 32) * 4; // UTF-8 decoding
          }
          else {
            fi += (str[a] - 32) * 4;
          }
    ...
    if (font_char >= 32)    // it is a printable char
    {
      if (_f_utf8) {
        if ((font_char >= 0xC2) && (font_char < 0xd5)) {
          if ((font_char == 0xd4) && (str[i + 1] > 0xAF)) {} // do nothing, it is a armenian character or higher
            ...
            xC = (font_char - 0xC2) * 64;
            i++;
            font_char = str[i] + xC; // UTF-8 decoding
          }
      ...
      if (font_char == 0) space = font_height / 4;
      else space = 0; //correct spacewidth is 1
      if (_textorientation == 0) {
        if ((Xpos + char_width + space) >= width()) {
          Xpos = tmp_curX;
          Ypos += font_height;
          Xpos0 = Xpos;
          Ypos0 = Ypos;
        }
        if ((Ypos + font_height) >= height()) {
          tmp_curX = Xpos;
          tmp_curY = Ypos;
          endWrite();
          return i;
        ...
        if ((Ypos + char_width + space) >= height()) {
          Ypos = tmp_curY;
          Xpos -= font_height;
          Xpos0 = Xpos;
          Ypos0 = Ypos;
        }
        if ((Xpos - font_height) < 0) {
          tmp_curX = Xpos;
          tmp_curY = Ypos;
          endWrite();
          return i;
      ...
      if (_textorientation == 0) {
        Ypos = Ypos0;
        Xpos0 = Xpos0 + char_width + 1 + space;
        Xpos = Xpos0;
      } else {
        Xpos = Xpos0;
        Ypos0 = Ypos0 + char_width + 1 + space;
        Ypos = Ypos0;
      }
    } // end if(font_char>=0)
...
size_t TFT::write(const uint8_t *buffer, size_t size) {
  if (_f_cp1251) {
    writeText(UTF8toCp1251(buffer));
    return 0;
  }
  if (_f_cp1252) {
    writeText(UTF8toCp1252(buffer));
    return 0;
  }
  if (_f_cp1253) {
    writeText(UTF8toCp1253(buffer));
    return 0;
  }
  writeText(buffer);
  return 0;
...
bool TFT::GIF_ReadImage(uint16_t x, uint16_t y) {
  ...
  while (count != max) {
    ...
    count++;
    xpos++;
    if (xpos == (x + gif_ImageWidth + gif_ImageLeftPosition)) {
      xpos = x + gif_ImageLeftPosition;
      ypos++;
    }
  }
  endWrite();
...
void JPEGDecoder::transformBlock(uint8_t mcuBlock) {
  ...
  switch (gScanType) {
    case PJPG_GRAYSCALE: {
        copyY(0);  // MCU size: 1, 1 block per MCU
        break;
      }
...
void JPEGDecoder::transformBlockReduce(uint8_t mcuBlock) {
  ...
  switch (gScanType) {
    case PJPG_GRAYSCALE: {
        gMCUBufR[0] = c;  // MCU size: 1, 1 block per MCU
        break;
      }
    case PJPG_YH1V1: {                              // MCU size: 8x8, 3 blocks per MCU
        switch (mcuBlock) {
...
void TP::loop() {
  if (!digitalRead(TP_IRQ)) {
    ...
    if (f_loop) {
      f_loop = false;
      if (read_TP(x, y)) {
        if (tp_pressed) tp_pressed(x, y); //zweite Messung lesen
      }
    }
  } else {
    if (f_loop == false) {
