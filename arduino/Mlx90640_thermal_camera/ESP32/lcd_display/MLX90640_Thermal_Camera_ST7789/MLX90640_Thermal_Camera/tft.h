...
#include "Garamond.h"               // default font latin (Western European)
...
class TFT : public Print {
  ...
  public:
    ...
    void      begin(uint8_t CS = 15, uint8_t DC = 2, uint8_t MOSI = 23, uint8_t MISO = 19, uint8_t SCK = 18, uint8_t BL = 17);
  ...
  private:
    ...
    // uint8_t  TFT_DC  = 2;    /* Data or Command */
    // uint8_t  TFT_CS  = 15;    /* SPI Chip select */
    ...
    uint8_t  TFT_DC;    /* Data or Command */
    uint8_t  TFT_CS;    /* SPI Chip select */
    uint8_t  TFT_BL;    /* BackLight */
    uint8_t  TFT_SCK;
    uint8_t  TFT_MISO;
    uint8_t  TFT_MOSI;
    uint8_t  TFT_RST;   /* Reset */
